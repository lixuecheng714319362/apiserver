...
//copyrightchain/go /root/projectMod/apiserver/chaincode

func main()  {
	args:=os.Args
	if len(args)!=3{
		fmt.Println("error: args number error")
		return
	}
	ccp,err:=NewCCPackage(args[1],args[2])
	if err != nil {
		fmt.Println("error: get package error")
		return
	}
	cp,err:=json.Marshal(ccp)
	if err != nil {
		fmt.Println("error: json marshall error")
		return
	}
